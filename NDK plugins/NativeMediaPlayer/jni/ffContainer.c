...
#include <prebuilt/armeabi-v7a/include/renderplugin.h>
...
static int initPlayer(const char* _fName) {
	LOGD("Beggining intialisation");
	info = malloc(sizeof(VideoInfo));
	info->name = (char*)_fName;
	av_register_all();
	VideoState *vs;
	...
	int res = avformat_open_input(&vs->pFormatCtx, info->name, NULL, NULL);
	if (res != 0) {
		LOGE("Error opening video file");
		return (res);
	}
	...
	av_dump_format(vs->pFormatCtx, 0, info->name, 0);
	...
	//Copy context
	vs->pCodecCtx = avcodec_alloc_context3(vs->pCodec);
	...
	//Get video info.
	info->height = vs->pVideoStream->codec->height;
	info->width = vs->pVideoStream->codec->width;
...
extern int DecodeFrame() {
	...
	sws_ctx = sws_getContext(info->width, info->height, vs->pCodecCtx->pix_fmt,	info->width, info->height, PIX_FMT_RGB24, SWS_BILINEAR,	NULL, NULL,	NULL);
	...
	while (av_read_frame(vs->pFormatCtx, &packet) >=0) {
		...
		if (packet.stream_index == vs->videoStreamIdx) {
			...
			//Decode frame
			avcodec_decode_video2(vs->pCodecCtx, pFrame, &frameFinished, &packet);
